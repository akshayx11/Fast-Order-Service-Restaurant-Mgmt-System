...
int tableno= Integer.parseInt(request.getParameter("ctableno"));
...
/*if(qty[i]!=0)
{
    ...
    ttime1= time[1]+time[2]+time[3]+time[4]+time[5]+time[6]+time[7]+time[8]+time[9]+time[10]+time[11]+time[12];
      ttime1= time[12]+time[14]+time[15]+time[16]+time[17]+time[18]+time[19]+time[20]+time[21];
            ...
            out.println(" <b>Branch:</b>IT(A) 3rd yr</br>");
            ...
            PreparedStatement ps= con.prepareStatement("insert into fos(tableno,itemname,qty,time,total) values(?,?,?,?,?)");
         ...
         /*   PreparedStatement ps2= con.prepareStatement("update baltab set trnsid=? WHERE id = 1");
               {     int p = ttrns+3;
            ps2.setInt(1,p);
            ps2.setDouble(2,bbal+pay1);
